...
# trade_intent is scaffolded — recognized by the sync/storage layer but has no
# writer anywhere yet, so it always produces an empty delta (cheap no-op).

CROSS_PLATFORM_DIMS: frozenset[str] = frozenset({"commodity", "city", "state", "trade_intent"})

# ── Confidence thresholds ─────────────────────────────────────────────────────

CATEGORY_CONF_THRESHOLD: float = 10.0
AUTHOR_SESSION_CONF_THRESHOLD: float = 6.0
