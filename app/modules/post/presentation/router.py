...
async def create_post_api(
    ...
    try:
        result = await service.create_post(repo, profile_id, payload)
...
#     repo: IPostRepository = Depends(get_post_repo),
# ):
#     result = service.get_feed(repo, profile_id, limit, offset)
...
def get_my_posts_api(
    ...
    return service.get_my_posts(repo, profile_id, limit, cursor)
...
def get_following_feed_api(
    ...
    return service.get_following_feed(repo, profile_id, limit, cursor)
...
def get_saved_posts_api(
    ...
    return service.get_saved_posts(repo, profile_id, limit, cursor)
...
def update_post_api(
    ...
    try:
        result = service.update_post(repo, post_id, profile_id, payload)
...
async def delete_post_api(
    ...
    try:
        await service.delete_post(repo, post_id, profile_id)
    except service.PostNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except service.PostForbiddenError as e:
        raise HTTPException(status_code=403, detail=str(e))
...
def get_comments_api(
    ...
    try:
        return service.get_comments(repo, post_id, limit, cursor)
    except service.PostNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
def delete_comment_api(
    ...
    try:
        service.delete_comment(repo, post_id, comment_id, profile_id)
...
def toggle_deal_closed_api(
    ...
    try:
        result = service.toggle_deal_closed(repo, post_id, profile_id)
