# Post Module API Contract — v3

> Reflects codebase state as of 2026-06-02.
>
> **Changes from v2:**
> - `GET /posts/` (chronological all-posts feed) commented out — no active use case yet.
> - `GET /posts/recommendation/feed` now returns full post + author data in every card (no separate hydration step needed).
> - Recommendation feed response no longer requires a follow-up `GET /posts/{id}` per card.
> - `POST /posts/recommendation/seen` added — frontend tells the backend which posts are seen; backend no longer marks seen at feed delivery time.
> - `GET /posts/{post_id}` now also marks the post as seen (opening = definitive seen signal).
> - `RecommendedPost` schema replaced by `FeedPostCard`.
...
### PostAuthorResponse
Embedded inside `FeedPostCard`. Contains the fields needed to render an author card on the feed.
...
```json
{
  ...
  "score":  "float"             // recommendation engine score — informational
}
```
...
| status | reason |
|--------|--------|
| 403 | not the post owner, post is not category 4, or deal_details row is missing |
| 404 | post not found |
...
Returns a personalised feed of up to 25 posts. Each card includes the full post data and author profile — no follow-up requests needed to render a feed card.

**Scoring factors:** vector similarity (commodity, role, geo, qty) × taste weight × engagement × freshness × social graph boost.

**Exclusions:** closed deals, posts seen in last 30 days.
...
Mark posts as seen. Frontend decides when a post qualifies as seen (scroll-past dwell, explicit open, etc.). Posts will not appear in recommendation feed for 30 days from first seen call.

**Note:** Opening a post via `GET /posts/{post_id}` also marks it as seen automatically.
...
| Event | Effect on seen_posts |
|-------|----------------------|
| `POST /posts/recommendation/seen` with `post_ids` | Written immediately |
| `GET /posts/{post_id}` (open post) | Written automatically |
| Post appears in feed but no signal sent | Not written — post can reappear |
...
7. A deal post removed from the recommendation pool (closed or deleted) is not shown in feeds but remains accessible via direct `GET /posts/{id}`.
